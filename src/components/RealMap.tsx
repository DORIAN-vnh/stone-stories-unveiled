...
import { MapPin, Mountain, Building2, Gem, Navigation } from 'lucide-react';
    ...
    <div className="leaflet-top leaflet-right">
      <div className="leaflet-control leaflet-bar">
        <Button
          size="sm"
          onClick={locateUser}
          className="rounded-none first:rounded-t last:rounded-b"
        >
              ...
              <Popup>
                <div className="p-2 min-w-[200px]">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <Badge variant="secondary" className={`${getColorForType(location.type)} text-white text-xs`}>
                      {location.type}
                    </Badge>
                  </div>
                  <p className="text-sm text-gray-600 mb-2">{location.description}</p>
                  {location.material && (
                    <p className="text-xs"><strong>Material:</strong> {location.material}</p>
                  )}
                  {location.era && (
                    <p className="text-xs"><strong>Era:</strong> {location.era}</p>
                  )}
                  {location.company && (
                    <p className="text-xs"><strong>Info:</strong> {location.company}</p>
                  )}
                </div>
              </Popup>
            </Marker>
